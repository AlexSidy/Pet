...
Качество кода: Использование SonarQube для анализа качества кода, а та же code coverage с 80% требованием.
...
ScanPerson Service: содержит интерфейс для взаимодействия через браузер и Api для взаимодействия, а так же основную бизнес логику проекта.

Graylog Service: Выделен в отдельный солюшн для централизованного логирования.

Identity Service: Реализован как отдельный солюшн для управления аутентификацией и авторизацией. В будущем планируется создание HTTP-клиента для взаимодействия с ним.

Common Services: Сервисы общего назначения, такие как PgAdmin (для управления базами данных), Redis, RabbitMQ, будут собраны в отдельный "common" солюшн.
...
Выберите свой путь к файлу и не забудьте обновить путь и пароль к сертификату в файле .env.
...
### Лицензия 📜
Этот проект распространяется под лицензией MIT. Подробнее см. файл [LICENSE](https://github.com/AlexSidy/Pet/blob/dev/LICENSE.md).
